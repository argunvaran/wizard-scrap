...
class EnglandStandingsScraper(BaseScraper):
    def scrape(self, url="https://www.mackolik.com/puan-durumu/ingiltere-premier-lig/2kwbbcootiqqgmrzs6o5inle5"):
        ...
        try:
            ...
            for i, table in enumerate(tables):
                ...
                print(f"Checking table {i} headers: {cleaned_headers}")
                if 'O' in cleaned_headers and 'P' in cleaned_headers:
                    target_table = table
                    break
            
            if not target_table:
                print("Standings table not found! Headers didn't match requirements.")
                # Fallback to first table if O/P check failed but table exists
                if tables:
            ...
            for i, row in enumerate(rows):
                ...
                try:
                    ...
                    if len(text_cells) == 12:
                        # Explicit 12-column 'Ingiltere' layout:
                        # 0: Rank, 1: Logo, 2: Team, 3: Played, 4: +/-, 5: Won, 6: Drawn, 7: Lost, 8: GF, 9: GA, 10: Avg, 11: Points
                        played = text_cells[3]
                        won = text_cells[5]
                        drawn = text_cells[6]
                        lost = text_cells[7]
            ...
            print(f"Error in EnglandStandingsScraper: {str(e)}")
        finally:
            self.close_browser()
