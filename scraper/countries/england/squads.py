...
class EnglandSquadsScraper(BaseScraper):
    def scrape(self, custom_url=None):
        # We generally use the JSON flat file source for squads
        links_path = "c:/Code/web_scraper_0/data/england_team_links.json"
        
        if not os.path.exists(links_path):
            links_path = os.path.join(os.getcwd(), "data", "england_team_links.json")
            if not os.path.exists(links_path):
        ...
        try:
            ...
            for i, team in enumerate(teams):
                ...
                for table in tables:
                    ...
                    # Check if this looks like a player table (usually has many rows)
                    ...
                    for row in rows:
                        ...
                        if link_el.count() > 0:
                            ...
                            # We populate Col_X keys regardless, to ensure fallback logic works for hidden/icon headers
    ...
    def save_json(self, data):
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        path = os.path.join(folder, "england_squads_flat.json")
