...
class SpainSquadsScraper(BaseScraper):
    def scrape(self, custom_url=None):
        try:
            ...
            logger.info(f"Looking for Spain team links at: {links_path}")
        ...
        try:
            ...
            for i, team in enumerate(teams):
                ...
                for table in tables:
                    ...
                    if len(rows) < 3: continue # Skip non-player tables
    ...
    def _parse_row(self, row):
        ...
        # Multilingual Mapping (TR/EN/ES potentially)
        key_map = {
            "Team": "team_name",
            "Player": "player_name", "Ad": "player_name", "Nombre": "player_name",
            "Profile_URL": "profile_url",
            "Forma": "jersey_number", "No": "jersey_number", "Num": "jersey_number",
            "Pozisyon": "position", "POZ": "position", "Pos": "position",
            "Ülke": "nationality", "Uyruk": "nationality",
            "Yaş": "age", "Age": "age", "Edad": "age",
        ...
        # 2. Fallback to Indices based on col_count
    ...
    def save_json(self, data):
        ...
        if not os.path.exists(folder):
            os.makedirs(folder)
        path = os.path.join(folder, "spain_squads_flat.json")
        with open(path, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=4)
        print(f"Saved {len(data)} Spain player records to {path}")
